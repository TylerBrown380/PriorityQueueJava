...
class CPUScheduling {
	public static void main(String[] args) {
		if (args.length != 5) {
			...
			System.out.println("<max-process-time>: max number of unit time for executing a process.");
			System.out.println("<max-level>: priority levels are 1, 2, .. <max-level>");
			System.out.println(
			...
			System.out.println("<simulation-time>: number of unit time for this simulation.");
			System.out.println("<process arrival rate>: the process arrival probability within each time unit.");
			System.exit(1);
		}

		int maxProcessTime = Integer.parseInt(args[0]);
		if (maxProcessTime <= 0)
			throw new IllegalArgumentException("Illegal argument: maxProcessTime must >= 1.");
		int maxLevel = Integer.parseInt(args[1]);
		if (maxLevel <= 0)
			throw new IllegalArgumentException("Illegal argument: maxPriorityLevel must >= 1.");
		...
		if (simulationTime <= 0)
			throw new IllegalArgumentException("Illegal argument: simulationTime must >= 1.");
		...
		System.out.println("Average Turn around time for a process: " + averager.average());
	}
}